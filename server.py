...
# Start the server.
def run_server(hostname, server_port, raft_port, super_node_address, lru_capacity=50):
    
    #  Restricts the instantiation of a class to one "single" instance. 
    # CRITICAL: Pass current node address to ensure it's always counted as active
    current_node_address = hostname + ":" + server_port
    activeNodesChecker = ActiveNodesChecker(current_node_address=current_node_address)
    ...
    # Declare the gRPC server with 200 max_workers for high concurrency
    # Critical: Increased from 10 to handle 200+ concurrent stress test users
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=200))

    # Add FileService to the server.
    fileService_pb2_grpc.add_FileserviceServicer_to_server(FileServer(hostname, server_port, activeNodesChecker, shardingHandler, super_node_address, lru_capacity), server)
    ...
    # Use port = server_port + 10000 for metrics endpoint
    metrics_port = int(server_port) + 10000
    start_metrics_server(metrics_port)
    print(f"Prometheus metrics available at http://localhost:{metrics_port}/metrics")
    print(f"Node running on gRPC port {server_port}, Raft port {raft_port}, Metrics port {metrics_port}")
...
if __name__ == '__main__':
    ...
    lru_capacity = config_dict_orig.get('LRUCapacity', 50)

    #Initially set the primaryStatus to 0
    db.setData("primaryStatus", 0)

    # Start the server
    run_server(server_host, server_port, raft_port, super_node_address, lru_capacity)